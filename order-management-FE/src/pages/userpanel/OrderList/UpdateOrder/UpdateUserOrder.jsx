...
import {
  validateOrderForm,
  validateField,
} from "../../CreateOrder/OrderValidation";
import {
  fetchCompanies,
  fetchDepartments,
...
const UpdateUserOrder = () => {
  ...
  useEffect(() => {
    ...
    if (orderById && Object.keys(orderById).length > 0) {
      // Transform products array to match your form structure
      const transformedProducts = orderById.products.map((product) => ({
      ...
      // Set form data with order values
      setFormData((prev) => ({
        ...prev,
        ...
        deliveryLocation: orderById.location || "",
        changeLocation: orderById.change_location || false,
  ...
  // Also, you might want to fetch departments and cost centers when company/department are set
  useEffect(() => {
    if (
      orderById &&
      ...
      const filtered = departments.filter(
        (dept) => dept?.company_id?.name === orderById.company_name
      );
      setShownDepartments(filtered);

      // If department exists, fetch cost centers
      if (orderById.department_name) {
        const selectedDept = departments.find(
          (dept) => dept.name === orderById.department_name
        );
  ...
  }, [orderById, departments, dispatch]);
  ...
  const handleAddProduct = () => {
    ...
    if (editingIndex >= 0) {
      ...
      updatedProducts[editingIndex] = {
        ...currentProduct,
        replace_product_id: currentProduct.product_id,
      };
  ...
  const handleEditProduct = (index) => {
    ...
    setCurrentProduct({ ...product, replace_product_id: product.product_id });
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      await dispatch(
        updateOrderById({ orderId: orderId, orderData: form })
      ).then(() => {

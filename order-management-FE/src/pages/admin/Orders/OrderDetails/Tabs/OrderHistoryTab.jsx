...
export const OrderHistoryTab = ({ orderById, viewProduct }) => {
  ...
  const ownsStatus = (step, status) => STEP_OWNER[step]?.includes(status);
  ...
  return (
    <div className="mt-10 grid grid-cols-1 md:grid-cols-2 justify-between gap-5">
      ...
      <div className="relative pl-8 flex flex-col">
        {/* ─────── Created ───────────────────────────────────────────── */}
        <div
          className={`pb-8 relative before:absolute before:-left-6 before:top-0 h-[170px] before:h-[calc(100%+1px)] before:w-[1px] ${CREATED_COMPLETE.includes(viewProduct.status)
            ? "before:bg-[#2EA0AC]"
            : "before:bg-[#888888]"
            }`}
        >
          ...
          <div className="mt-2.5 flex flex-col gap-1">
            <p className="text-sm text-[#212121]">
              Order for{" "}
              {viewProduct?.product?.product_name || viewProduct?.product_name}{" "}
              has been created.
            </p>
            ...
            <p className="text-xs text-[#6D6D6D]">
              Created By: {orderById?.created_by?.name || orderById?.created_by}
            </p>
            <p className="text-xs text-[#6D6D6D]">
        ...
        {REJECT_OR_CANCEL.includes(viewProduct.status) ? (
          ...
          <>
            {/* ─── Approval ─────────────────────────────────────────── */}
            <div
              className={`pb-8 relative before:absolute before:-left-6 before:top-0 h-[200px] before:h-[calc(100%+1px)] before:w-[1px] ${APPROVAL_COMPLETE.includes(viewProduct.status)
                ? "before:bg-[#2EA0AC]"
                : "before:bg-[#888888]"
                }`}
            >
              ...
              <div className="mt-2 text-sm text-gray-600">
                {ApprovalDes ? (
                  <>
                    ...
                    <p className="text-xs text-[#6D6D6D]">
                      {ownsStatus("Approval", viewProduct.status) &&
                        viewProduct.status}
                    </p>
                  </>
                ) : (
            ...
            >
              ...
              {deliveryDes ? (
                <>
                  ...
                  <p className="text-xs text-[#6D6D6D]">
                    {formatDate(
                      deliveryDes?.changed_at
                    )}
                  </p>
                  <p className="text-xs text-[#6D6D6D]">
                    {ownsStatus("Delivery", viewProduct.status) &&
                      viewProduct.status}
                  </p>
                </>
              ) : (
            ...
            <div className="relative">
              ...
              {completedDes ? (
                <>
                  ...
                  <p className="text-xs text-[#6D6D6D]">
                    {ownsStatus("Completed", viewProduct?.status) &&
                      viewProduct?.status}
                  </p>
                </>
              ) : (
      ...
      <div className="w-full bg-white shadow-[0px_4px_16px_0px_#0000000D] border border-[#E7E7E7] rounded-[20px] justify-between overflow-hidden p-5 flex flex-col gap-4">
        <div className="flex-1 flex flex-col justify-start">
          <h2 className="text-base font-medium text-[#212121]">All Comments</h2>

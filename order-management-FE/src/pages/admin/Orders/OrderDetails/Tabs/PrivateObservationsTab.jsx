...
export const PrivateObservationsTab = ({ orderById, viewProduct }) => {
  const dispatch = useDispatch();
  const [replyText, setReplyText] = useState("");
  const [comments, setComments] = useState(viewProduct?.private_comments || []);
  const loggedUser =
  ...
  const handleSendComment = async () => {
    ...
    const data = { message: replyText, product_id: viewProduct?.product_id };

    try {
      const res = await dispatch(
        postPrivateComment({ id: orderById._id, data })
      ).unwrap();
      if (res && res.comment) {
  ...
  const handleLikeComment = async (commentId) => {
    try {
      let data={product_id: viewProduct?.product_id}
      const res = await dispatch(
        likePrivateComment({ orderId: orderById._id, commentId: commentId, data })
      ).unwrap();
      if (res && res.comment) {
